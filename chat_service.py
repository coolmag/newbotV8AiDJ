...
class ChatManager:
    ...
    async def get_response(chat_id: int, user_text: str, user_name: str) -> str:
        ...
        # Формируем историю сообщений в формате GigaChat
        # (System сообщение добавляем первым)
        ...
        # 1. GIGACHAT (PRIORITY)
        # Получаем ключи из переменных окружения (GIGACHAT_CREDENTIALS)
        giga_key = os.getenv("GIGACHAT_CREDENTIALS")
        
        if HAS_GIGACHAT and giga_key:
            try:
                # ВАЖНО: verify_ssl_certs=False для работы в контейнере без сертификатов МинЦифры
                ...
                # Запускаем синхронный SDK в потоке
