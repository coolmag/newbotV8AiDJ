...
class YouTubeDownloader:
    """
    ADAPTER: RAILWAY 'HYBRID' STABLE (2026)
    - Piped API fallback.
    ...
    async def download(self, video_id: str, track_info: Optional[TrackInfo] = None) -> DownloadResult:
        # FIX FOR WEB PLAYER: If called without track_info, create a stub
        if not track_info:
        ...
        # THE REAL FIX: Construct a new, clean object before returning.
        if result.success:
    ...
    async def _download_piped(self, video_id: str, track_info: Optional[TrackInfo]) -> DownloadResult:
        # This list is now configurable via config.py
