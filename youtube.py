...
class YouTubeDownloader:
    ...
    def __init__(self, settings: Settings, cache_service: CacheService):
        ...
        self.token = os.getenv("VK_TOKEN")  # remixsid токен
        ...
        if self.token:
            ...
            logger.critical("⛔️ VK_LOGIN+VK_PASSWORD или VK_TOKEN не найдены!")

    def _auth_token(self):
        """Авторизация через готовый remixsid токен."""
        try:
            logger.info("🔐 VK авторизация через TOKEN (remixsid)...")
            ...
            self.is_active = True
            logger.info("✅ VK Music Engine: ONLINE (Token Auth)")
            
        except vk_api.VkApiError as e:
            logger.error(f"❌ VK Token Error: {e}")
            self.is_active = False
    ...
    def _auth_password(self):
        """Авторизация через логин/пароль (только без 2FA)."""
        try:
            logger.info(f"🔐 VK авторизация: {self.login[:6]}***")
            ...
            self.vk_audio = VkAudio(self.vk_session)
            self.is_active = True
            logger.info("✅ VK Music Engine: ONLINE (Password Auth)")
            
        except vk_api.AuthError as e:
            error_msg = str(e)
            if "code" in error_msg.lower() or "2fa" in error_msg.lower():
                logger.error("""
❌ VK ТРЕБУЕТ 2FA!
   
   Решение:
   1. Отключи "Подтверждение входа" в VK
   2. ИЛИ используй VK_TOKEN (remixsid из браузера)
   
   Как получить TOKEN:
   1. Зайди в vk.com через браузер
   2. F12 → Application → Cookies → remixsid
   3. Добавь переменную VK_TOKEN в Railway
                """)
            else:
    ...
    def _captcha_handler(self, captcha):
        """Обработчик капчи."""
        logger.warning("⚠️ Капча получена")
        return None
    ...
    async def search(self, query: str, search_mode: str = 'genre', decade: Optional[str] = None, limit: int = 20) -> List[TrackInfo]:
        if not self.is_active:
            logger.warning("VK not active (check auth)")
            return []
