...
import logging
import random
import inspect  # <--- Для проверки аргументов
...
class YouTubeDownloader:
    """
    🎵 YouTube Music Edition (v45 - Universal Fix).
    Fixes: Dynamically detects TrackInfo arguments to prevent errors.
    """
    ...
    async def search(self, query: str, limit: int = 10, **kwargs) -> List[TrackInfo]:
        ...
        try:
            ...
            # Определяем, какие поля ждет TrackInfo
            sig = inspect.signature(TrackInfo)
            has_uploader = 'uploader' in sig.parameters
            has_artist = 'artist' in sig.parameters
            ...
            for item in search_results:
                ...
                # Подставляем артиста в нужное поле
                if has_uploader: track_args['uploader'] = artists
                elif has_artist: track_args['artist'] = artists
                elif has_author: track_args['author'] = artists
                
                track = TrackInfo(**track_args)
    ...
    async def download(self, video_id: str, track_info: Optional[TrackInfo] = None) -> DownloadResult:
        ...
        try:
            ...
            if proxy_started: self._proxy_manager.stop_proxy()
    ...
    async def _download_smart(self, video_id: str, target_path: Path, track_info: TrackInfo) -> DownloadResult:
        ...
        clients = [['android', 'android_music'], ['ios'], ['tv_embedded', 'web_creator']]
