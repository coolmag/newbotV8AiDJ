...
class YouTubeDownloader:
    ...
    def __init__(self, settings: Settings, cache_service: CacheService):
        ...
        # --- LAST CHANCE CONFIG ---
        self.ydl_opts = {
            # Приоритет: Лучшее видео+аудио, или лучшее аудио.
            # Это обходит блок "audio-only" запросов.
            ...
            "postprocessors": [{
                ...
                'preferredcodec': 'mp3',
                'preferredquality': '128', # 128kbps достаточно и быстрее качается
            }],
            ...
            # Упрощенные аргументы (меньше триггеров защиты)
            "extractor_args": {
                "youtube": {
    ...
    async def search(self, query: str, search_mode: str = 'genre', decade: Optional[str] = None, limit: int = 20) -> List[TrackInfo]:
        async with self.search_semaphore:
            ...
            cache_key = f"yt_search_v23:{clean_query}"
            cached = await self._cache.get(cache_key)
            if cached: return cached

            suffixes = ["", " music", " official"]
            all_tracks = []
