    ...
    async def _radio_loop(self):
        ...
        while self.is_running:
            try:
                ...
                try:
                    # Увеличил таймаут загрузки/отправки, чтобы не падать на больших файлах
                    success = await asyncio.wait_for(self._play_track(track), timeout=180.0)
                    
                    if success:
                        error_streak = 0
                        self.tracks_played += 1
                        
                        # === ВОТ ЗДЕСЬ НАСТРОЙКА ВРЕМЕНИ (120 секунд = 2 минуты) ===
                        try:
                            await asyncio.wait_for(self.skip_event.wait(), timeout=120.0)
                        except asyncio.TimeoutError:
                            # Это НОРМАЛЬНОЕ поведение: время вышло, переходим к следующему треку
                            pass 
                            
                    else: 
                        raise Exception("Play track failed")
        ...
        try:
            ...
            if base_url.startswith("https") and self.chat_type != ChatType.CHANNEL:
                if self.chat_type == ChatType.PRIVATE:
                    ...
                    # В группах обычная ссылка (WebApp кнопки там запрещены в таком виде)
                    markup = InlineKeyboardMarkup([
