...
class SpotifyService:
    ...
    def _initialize_spotify_client(self):
        ...
        if not client_id or not client_secret:
            logger.warning("⚠️ SPOTIFY_CLIENT_ID or SPOTIFY_CLIENT_SECRET not set. Spotify features will be limited.")
            return
    ...
    async def download_spotify_track(self, spotify_url: str) -> DownloadResult:
        ...
        try:
            ...
            yt_tracks = await self._yt_downloader.search(query, limit=1)
            ...
            # Use the Spotify metadata for the final TrackInfo object
            final_track_info = TrackInfo(
                identifier=best_yt_track.identifier,
                title=track_name,
                artist=artist_name,
                duration=best_yt_track.duration,
                source=Source.SPOTIFY, # Indicate origin as Spotify
                thumbnail_url=spotify_track['album']['images'][0]['url'] if spotify_track['album']['images'] else None
            )

            logger.info(f"Downloading '{final_track_info.title}' from YouTube (via Spotify).")
            ...
            return DownloadResult(success=False, error_message=f"Internal error processing Spotify track: {e}.")
